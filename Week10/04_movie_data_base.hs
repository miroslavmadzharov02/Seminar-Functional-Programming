main :: IO()
main = do
    print $ getFeaturedStars "MGM" 1995 database == ["Jack Nicholson", "Sandra Bulloc"]
    print $ getFeaturedStars "USA Entertainm." 2001 database == ["Billy Bob Thornton", "Scarlett Johansson", "Orlando Bloom", "Cate Blanchett", "Liv Tyler"]

    print $ getPresident "Paramount" database == "Calvin Coolidge"
    print $ getPresident "Fox" database == "Ted Turner"
    print $ getPresident "USA Entertainm." database == "Stephen Spielberg"

getFeaturedStars :: Name -> Year -> MovieDB -> [Name]
getFeaturedStars company releaseYear (ms, _, si, _, _) = concatMap getActors getMovies
    where
        getMovies :: [Title]
        getMovies = [title | (Movie title year _ studioName _) <- ms, studioName == company && releaseYear == year]

        getActors :: Title -> [Name]
        getActors movieTitle = [actorName | (StarsIn actorName title) <- si, title == movieTitle]
...
database :: MovieDB
database = (movies, stars, starsIn, studios, movieExecs)